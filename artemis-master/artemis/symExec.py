...
from vulnerability import CallStack, SuicideContract, GreedyContract, TimeDependency,txOriginDependency, MoneyConcurrency, Reentrancy, AssertionFailure, ParityMultisigBug2, IntegerUnderflow, IntegerOverflow,bInfoDependency, GaslessSend, DangerousDelegatecall
...
def initGlobalVars():
    ...
    global flag_suicide
    flag_suicide = 0
    global gasless_send_flag
    gasless_send_flag = 1
    global recipients
    recipients=set()
    global balance_greedy
    balance_greedy = 0
    global balance_ia
    balance_ia=0
    global  new_balance_ia
    new_balance_ia=0
    global num_nonpayable
    num_nonpayable=0
    global num_function
    num_function=0
    global fbbegin
    fbbegin = 0
    global fallback_sort
    fallback_sort = 0
    global fbend
    fbend = 0
    global totalgas
    totalgas = 0
    global flag_callvalue
    flag_callvalue = 0
    global flag_binfo
    flag_binfo = 0 
    global gas_in_fallback
    gas_in_fallback = 0
    ...
    global instr_opcodes
    instr_opcodes = []
    
    global global_problematic_pcs
    global_problematic_pcs = {"money_concurrency_bug": [], "reentrancy_bug": [], "time_dependency_bug": [], "assertion_failure": [], "integer_underflow": [], "integer_overflow": [], "binfo_dependency": []}
...
def get_init_global_state(path_conditions_and_vars):
    ...
    if global_params.INPUT_STATE:
        with open('state.json') as f:
            ...
            if state["env"]["currentBlockhash"]:
                currentBlockhash = int(state["env"]["currentBlockhash"], 16)                
    ...
    currentBlockhash = BitVec(new_var_name, 256)
    path_conditions_and_vars[new_var_name] = currentBlockhash
  
    new_var_name = "some_var_binfo"
    currentBinfo = BitVec(new_var_name, 256)
    path_conditions_and_vars[new_var_name] = currentBinfo
    ...
    global_state["currentBinfo"] = currentBinfo
    global_state["currentBlockhash"] = currentBlockhash
...
def sym_exec_block(params, block, pre_block, depth, func_call, current_func_name):
    ...
    if gasless_send_flag == 1 and current_gas_used >= 2300 :
        gasless_send_flag = 0
        # log.info("gas_used:%s",current_gas_used)
        gas_in_fallback = current_gas_used
        # log.info("\t\tFallback triggers gasless send bug!!")
    ...
    try:
        ...
        for instr in block_ins:
            instr_opcodes.append(str.split(instr, ' ')[0])
            if('NUMBER' in instr_opcodes and 'SHA3' in instr_opcodes):
                flag_binfo = 1 
    ...
    if analysis["money_flow"] not in money_flow_all_paths:
        ...
        # global_problematic_pcs["binfo_dependency"].append(analysis["binfo_dependency"])
    ...
    if jump_type[block] == "terminal" or depth > global_params.DEPTH_LIMIT:
        ...
        #solver.add(branch_expression == 0)
...
def sym_exec_ins(params, block, instr, func_call, current_func_name):
    ...
    #
        ...
        if len(stack) > 1:
            ...
            if isAllReal(s0, s1):
                ...
                if flag_binfo == 1:
                    new_var_name = gen.gen_arbitrary_var_binfo()
                    new_var = BitVec(new_var_name, 256)
                    path_conditions_and_vars[new_var_name] = new_var
                    stack.insert(0, new_var)                    
    ...
    #
        ...
        global_state["pc"] = global_state["pc"] + 1
        stack.insert(0, global_state["currentBlockhash"])
...
def detect_txOrigin_dependency():
    global results
    global g_src_map
    global txOrigin_dependency
    is_dependant = False
    ORIGIN_VAR = "Io"
    ORIGIN_VAR_FIRST= "Extract(159, 0, Io)"
    ORIGIN_VAR_SECOND = "Extract(159, 0, Ia_store"
    ...
    for i, cond in enumerate(path_conditions): 
        ...
        for j, expr in enumerate(cond):  
            if is_expr(expr):  
                if ORIGIN_VAR in str(expr) :#and j in global_problematic_pcs["txorigin_dependency"][i]: 
                    log.info("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~EXIST Io IN PATH_CONDITIONS")
                if ORIGIN_VAR_FIRST in str(expr) and ORIGIN_VAR_SECOND in str(expr):
                    pcs.append(1)
                    is_dependant = True
                    continue

    txOrigin_dependency = txOriginDependency(g_src_map, pcs)

    if g_src_map:
        results['vulnerabilities']['txOrigin_dependency'] = txOrigin_dependency.get_warnings()
    else:
        results['vulnerabilities']['txOrigin_dependency'] = txOrigin_dependency.is_vulnerable()
    log.info('\t  TxOrigin Dependency:    %s', txOrigin_dependency.is_vulnerable())
...
def detect_bInfo_dependency():
    global results
    global g_src_map
    global bInfo_dependency
    is_dependant = False
    NUMBER_VAR = "IH_i"
    DIFFICULTY_VAR = "IH_d"
    BLOCKHASH = "IH_blockhash"
    INFO = "some_var_binfo"
    pcs = []
    if global_params.PRINT_PATHS:
        log.info("ALL PATH CONDITIONS")
    for i, cond in enumerate(path_conditions): 
        ...
        for j, expr in enumerate(cond):  
            if is_expr(expr):  
                if DIFFICULTY_VAR in str(expr) or INFO in str(expr) or BLOCKHASH in str(expr) or NUMBER_VAR in str(expr):#  and j in global_problematic_pcs["binfo_dependency"][i]: 
                    pcs.append(1)
                    is_dependant = True
                    continue

    bInfo_dependency = bInfoDependency(g_src_map, pcs)

    if g_src_map:
        results['vulnerabilities']['bInfo_dependency'] = bInfo_dependency.get_warnings()
    else:
        results['vulnerabilities']['bInfo_dependency'] = bInfo_dependency.is_vulnerable()
    log.info('\t  BInfo Dependency2:      %s', bInfo_dependency.is_vulnerable())
...
def check_suicide_contract(disasm):
    global flag_suicide
    problematic_instructions = ['SUICIDE']
    pcs = []
    for i in range(0, len(disasm)):
        instruction = disasm[i]
        if (instruction[1] in problematic_instructions)  :
...
def detect_suicide_contract():
    global g_src_map
    global results
    global source_map
    global any_bug
    global suicide_contract

    disasm_data = open(g_disasm_file).read()
    instr_pattern = r"([\d]+) ([A-Z]+)([\d]+)?(?: => 0x)?(\S+)?"
    instr = re.findall(instr_pattern, disasm_data)
    pcs = check_suicide_contract(instr)   
    suicide_contract=SuicideContract(g_src_map, pcs)

    if g_src_map:
        results['vulnerabilities']['suicide_contract'] = suicide_contract.get_warnings()
    else:
        results["suicide contract"] = bool(pcs)
        results['vulnerabilities']['suicide_contract'] = suicide_contract.is_vulnerable()
    log.info("\t  Suicide Contract: \t\t\t %s", suicide_contract.is_vulnerable())

def fallback_type(disasm):
    global fbbegin
    global fbend
    global totalgas
    global fallback_sort
    ...
    for i in range(0, len(disasm)):
        instruction = disasm[i]   
        if  instruction[1] == 'PUSH' and instruction[2] == '2':
            if index == 0:
                fallback_begin = int(instruction[3],16)
            if index == 1:
                fallback_end = int(instruction[3],16)
            if index == 2:
                break
            index += 1
    ...
    for i in range(0, len(disasm)):
        instruction = disasm[i]
        if instruction[0] == str(fallback_end):
            fbend = i
        if instruction[0] == str(fallback_begin):
            fbbegin = i
    for i in range(fbbegin,fbend):
        instruction = disasm[i]
        if (fallback_end - fallback_begin == 5 ) and disasm[i+3][1] == 'REVERT' and disasm[i+2][1] == 'DUP' and disasm[i+1][1]=='PUSH':
        # fallback_sort is 0 stands for there is not fallback in this contract
            fallback_sort = 0 
            break
        # fallback_sort is 1 stands for there is a nonpayable fallback in this contract
        elif disasm[i+1][1] == 'CALLVALUE'  and disasm[i+2][1] == 'DUP' and disasm[i+3][1] == 'ISZERO':
            fallback_sort = 1
            break
        # fallback_sort is 2 stands for there is a payable fallback in this contract
        else:
            fallback_sort = 2 
            break 
    return fallback_sort

def check_gasless_send(disasm):
    global fallback_sort
    pcs = []
    fallback_sort = fallback_type(disasm)
    if (fallback_sort == 2) and (gas_in_fallback >2300):
        pcs.append(77)
    return pcs


def detect_gasless_send():
    global results
    global source_map
    global any_bug
    global balance_ia
    ...
    analysis = init_analysis()
    params = Parameter(path_conditions_and_vars=path_conditions_and_vars, global_state=global_state, analysis=analysis)
    pcs = check_gasless_send(instr)

    gasless_send=GaslessSend(g_src_map, pcs)
    if g_src_map:
        results['vulnerabilities']['gasless_send'] = gasless_send.get_warnings()
    else:
        results['vulnerabilities']['gasless_send'] = gasless_send.is_vulnerable()
    log.info("\t  Gasless Send:     %s", gasless_send.is_vulnerable())

def check_dangerous_delegatecall(disasm):
    pcs = []
    index = 0
    for i in range(0, len(disasm)):
        instruction = disasm[i]   
        if  instruction[1] == 'DELEGATECALL':
            if disasm[i+1][1] == 'ISZERO' and disasm[i+2][1] == 'ISZERO' and disasm[i+3][1] == 'PUSH' and disasm[i+3][2] == '2':
                continue
            if disasm[i+1][1] == 'ISZERO' and disasm[i+2][1] == 'PUSH' and disasm[i+2][2] == '2':
                continue
            if disasm[i+1][1] in ['INVALID','RETURN']:
                log.info(" invalid delegatecall")
                continue
            else:
                pc = int(instruction[0])
                pcs.append(pc)
...
def detect_dangerous_delegatecall():
    ...
    analysis = init_analysis()
    params = Parameter(path_conditions_and_vars=path_conditions_and_vars, global_state=global_state, analysis=analysis)
    pcs = check_dangerous_delegatecall(instr)

    dangerous_delegatecall = DangerousDelegatecall(g_src_map, pcs)
    if g_src_map:
        results['vulnerabilities']['dangerous_delegatecall'] = dangerous_delegatecall.get_warnings()
    else:
        results['vulnerabilities']['dangerous_delegatecall'] = dangerous_delegatecall.is_vulnerable()
    log.info("\t  Dangerous Delegatecall:    %s", dangerous_delegatecall.is_vulnerable())


def check_greedy_contract3(disasm,params):
    global data_source
    global balance_greedy
    global balance_ia
    global new_balance_ia
    ...
    '''
    if global_params.USE_GLOBAL_BLOCKCHAIN:
        balance_greedy = data_source.getBalance("0f572e5295c57f15886f9b263e2f6d2d6c7b5ec6")
        '''
    n = len(money_flow_all_paths)

    necessary_instructions = ['SUICIDE' , 'CALL' , 'DELEGATECALL' , 'CALLCODE']
    pcs = []
    for i in range(0, len(disasm)):
        instruction = disasm[i]
        if instruction[1] in necessary_instructions:
            return pcs
    fallback_sort = fallback_type(disasm)
    if fallback_sort == 1:
        num_nonpayable = -1
        for i in range(0, len(disasm)):
            instruction = disasm[i]
            if  instruction[1] == 'PUSH' and instruction[2] == '4':
                if disasm[i+1][1] == 'EQ' :
                    num_function+=1
            if instruction[1] == 'CALLVALUE' and disasm[i+1][1] == 'DUP' and disasm[i+2][1] == 'ISZERO':
                num_nonpayable += 1

            if (i==len(disasm)-1) and (instruction[1] not in necessary_instructions) and num_nonpayable != 0 and (num_nonpayable < num_function):  #合约balance值没有改变，即没有发钱的操作
                pc = int(instruction[0])
                pcs.append(pc)  
        return pcs
    elif fallback_sort == 2:
        pc = int(instruction[0])
        pcs.append(pc)
        return pcs
    elif fallback_sort == 0:
        for i in range(0, len(disasm)):
            instruction = disasm[i]
            if  instruction[1] == 'PUSH' and instruction[2] == '4':
                if disasm[i+1][1] == 'EQ' :
...
def detect_greedy_contract():
    ...
    analysis = init_analysis()
    params = Parameter(path_conditions_and_vars=path_conditions_and_vars, global_state=global_state, analysis=analysis)
    pcs = check_greedy_contract3(instr,params)
    greedy_contract=GreedyContract(g_src_map, pcs)
    if g_src_map:
        results['vulnerabilities']['greedy_contract'] = greedy_contract.get_warnings()
    else:
        results['vulnerabilities']['greedy_contract'] = greedy_contract.is_vulnerable()
    log.info("\t  Greedy Contract3:    %s", greedy_contract.is_vulnerable())
...
def detect_vulnerabilities():
    ...
    if instructions:
        ...
        detect_gasless_send()
        detect_greedy_contract()
        detect_bInfo_dependency()
        detect_txOrigin_dependency()
