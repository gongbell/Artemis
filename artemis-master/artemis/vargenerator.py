class Generator:
    ...
    def gen_arbitrary_var_binfo(self):
        self.count += 1
        return "some_var_binfo" 
        # + str(self.count)
    ...
    def gen_owner_store_var(self, position, var_name=""):
        return "Ia_store-%s-%s" % (str(position), var_name)  #生成保存owner地址的变量  生成的结果中一般var_name均为空
    ...
    def gen_caller_var(self):   #生成保存调用者地址的变量
        return "Is"
